...
export class OrthographicControls extends EventDispatcher {
    ...
    handleMouseDownPan( event ) {
        this.#ratioX = (this.#camera.left - this.#camera.right) / event.target.width;
        this.#ratioY = (this.#camera.top - this.#camera.bottom) / event.target.height;
    ...
    handleMouseWheel( event ) {
        ...
        let mouse = new THREE.Vector3(
            // (offsetX / event.target.width) * 2 - 1,
            // -(offsetY / event.target.height) * 2 + 1,
        ...
        this.#camera.position.x -= (this.#camera.position.x - mouse.x) * (1 - scale);
        this.#camera.position.y -= (this.#camera.position.y - mouse.y) * (1 - scale);
