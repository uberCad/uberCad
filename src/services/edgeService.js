...
let lineDivision = (editor, point, collisionPoints, threshold = 0.0001) => {
  ...
  point.entities.forEach((line, lineIndex) => {
    if (line.name !== 'changed line') {
      ...
      if (
        GeometryUtils.getDistance(point.point, linePoints[0]) > 0.1*threshold &&
        GeometryUtils.getDistance(point.point, linePoints[1]) > 0.1*threshold
      ) {
        ...
        if (newLines) {
          newLines[0].userData.needDelete = true;
          newLines[1].userData.needDelete = true;
          lineWithPoint.userData.dontUse = true;
          point.entities[lineIndex] = null;
          ...
          newLines[0].name = 'NEW LINE';
          newLines[1].name = 'NEW LINE';
...
const searchColPoints = (editor, svgForFlixo = false) => {
  const { scene } = editor;
  const threshold = editor.voidSearchOptions.ignoredDistance;
  ...
  collisionPoints.forEach(point => {
    ...
    point.entities.forEach((line, i) => {
      if (point.entities[i] !== null) {
        ...
        // костиль - проверка ростояния от линии до точки пересичения
        if (point.entities[i] !== null) {
          let distance;
  ...
  // поиск и отсев точкок которые расположены на ростоянии мение 0.01 от других точок
  ...
  // отсев точкок которые расположены на ростоянии мение 0.01 от других точок
  let lineIndex = 0;
  do {
  ...
  // поиск прилягающих линий к точкам collisionPoints
  collisionPoints.forEach(point => {
    point.entities.forEach(line => {
      ...
      if (
        ...
        line.parent.children.forEach(checkLine => {
          if (checkLine !== line && !point.entities.includes(checkLine)) {
  ...
  //   console.log(object.name + '  ' + object.children.length);
  // });
...
const combineEdgeModels = (editor, svgForFlixo = false) => {
  ...
  const objects = sceneService.getObjects(scene, true);
  let voidsLayer = scene.getObjectByName('VoidsLayer');
  voidsLayer.children = [];
  ...
  let collisionPoints = editor.voidSearchOptions.ColPoints;

  objects.forEach(object => {
    console.log(object.name + '  ' + object.children.length);
  });

  // раздиление линий на несколько в точках пересичения с друглими линиями
  collisionPoints.forEach((point, pointInd) => {
    lineDivision(editor, point, collisionPoints, 0.001);
  });

  // отсев старых(линий до роздиления) линий от сформированих относительно точок пересичения
  collisionPoints.forEach(point => {
    point.entities.forEach((line, lineIndex) => {
      if (line.userData.newLines) {
  ...
  objects.forEach(object => {
    ...
    do {
      if (object.children[lineIndex].userData.weDoneWithThisLine) {
        object.children[lineIndex].userData.weDoneWithThisLine = false;
      }
      if (object.children[lineIndex].userData.newLines) {
        object.children[lineIndex].userData.newLines = false;
      }
      if (object.children[lineIndex].userData.dontUse) {
        object.children[lineIndex].userData.dontUse = false;
      }
  ...
  let maxArea = 0;
  voids.forEach(object => {
    const geometryInfo = GeometryUtils.getObjectInfo(object);
    if (geometryInfo[0].area > allObjectArea && maxArea < geometryInfo[0].area){
      maxArea = geometryInfo[0].area;
    }
  });
...
const cloneObject = (editor, objects) => {
  editor.voidSearchOptions.ColPoints.forEach(point => {
    point.entities.forEach(line => {
  ...
  objects.forEach((object, index) => {
    ...
    console.log(object);
    object.children.forEach(origLine => {
      let newLine;
      let linePoints = sceneService.findWayPoint(origLine);
      if (origLine.geometry.type === 'Geometry') {
        newLine = createLine(linePoints[0], linePoints[1]);
      } else if (origLine.geometry.type === 'CircleGeometry') {
        newLine = GeometryUtils.newCurve(
          origLine.position,
          origLine.geometry.parameters.radius,
          linePoints[0],
          linePoints[1],
          editor
        );
      }
      newLine.parent = newLinesArr [index];
      newLinesArr[index].children.push(newLine);
      if (!newLine.parent) {
        debugger;
      }
      console.log (origLine.userData.collisionPointsInf);
      // debugger;
      if (origLine.userData.collisionPointsInf) {
        newLine.userData.collisionPointsInf = [];
        origLine.userData.collisionPointsInf.forEach(point => {
          newLine.userData.collisionPointsInf.push(point);
          point.entities.push(newLine);
        });
        origLine.userData.collisionPointsInf = [];
        // console.log (newLine.userData.collisionPointsInf);
        // console.log (origLine.userData.collisionPointsInf);
        // debugger;
      }
    });
...
const voindsInObject = (editor, object, line) =>{
  let lines = [];
  // let wayPoint = sceneService.findWayPoint(line);
  let checkLine = {
    line: line,
    index: 0,
    newFindLinePoint: sceneService.findWayPoint(line)
  };
  lines.push (line);
  line.userData.startFromThisPoint = true;
  let done = false;
  let pointIndex = 0;
  ...
  do {
    ...
    if (checkLine.line && !checkLine.line.userData.weDoneWithThisLine){
      ...
      if (checkLine.line.userData.startFromThisPoint === true){
        ...
        // checkLine.line.material.color.set(new THREE.Color(0xffaa00)); // оранжевий
        // sceneService.render(editor);
        lines.push (checkLine.line);
      }
    } else {
      lines = [];
      line.userData.startFromThisPoint = false;
      done = true;
    }
  } while (!done);
  return lines;
...
let searchTrueNextPoint = (
  ...
  if (lines_nextObject.length < 2) {
    let wayPoint = sceneService.findWayPoint(lines_nextObject[0])
    let newLine = findNextLine(
      lines_nextObject[0].parent,
      lines_nextObject[0],
      wayPoint[closestPoint(wayPoint, closesPoint.point)],
      1
    );
  }
  if (lines_nextObject.length > 2) {
  ...
  let point_D_nextPointOldObject =
    ...
    GeometryUtils.getDistance(closesPoint.point, pointsNextLine_oldObject[1])
  ...
  let point_A_newObject =
    ...
    GeometryUtils.getDistance(closesPoint.point, pointsWay_1_newObject[1])
  ...
  if (minDistance > 1.5) {
    ...
    if (
      ...
      let indexWay =
        ...
        GeometryUtils.getDistance(closesPoint.point, wayPoint[1]) ?
          0:1;
    ...
    if (
      ...
      let wayPoint = sceneService.findWayPoint(lines_nextObject[1]);
      let indexWay =
        GeometryUtils.getDistance(closesPoint.point, wayPoint[0]) >
        GeometryUtils.getDistance(closesPoint.point, wayPoint[1]) ?
          0:1;
    ...
    if (
      ...
      let wayPoint = sceneService.findWayPoint(nextLine_oldObject);
      let indexWay =
        GeometryUtils.getDistance(closesPoint.point, wayPoint[0]) >
        GeometryUtils.getDistance(closesPoint.point, wayPoint[1]) ?
          0:1;
    ...
    if (GeometryUtils.getDistance(closesPoint.point, point_E_oldLine) < 0.01) {
      let wayPoint = sceneService.findWayPoint(oldLine);
      let indexWay =
        GeometryUtils.getDistance(closesPoint.point, wayPoint[0]) >
        GeometryUtils.getDistance(closesPoint.point, wayPoint[1]) ?
          0:1;
    ...
    console.log(minDistance);
    if (minDistance > 1.5) {
      // todo возможно нужно пересмотреть алгоритм поиска пути
...
const findNextLine = (object, thisLine, linePoint, entrainment = 0.001) => {
  ...
  for (let i = 0; i < object.children.length; i++) {
    ...
    if (line !== thisLine && !line.userData.dontUse) {
      // if (line.geometry.type === 'Geometry') {
...
const nextPoint = (
  ...
  //
  if (!isVoid(thisLine, linePoint, editor)) {
    pieceOfFreeSpace[0] = [];
    return;
  }
  ...
  if (!pieceOfFreeSpace[0].includes(thisLine)) {
    ...
    // return;
    console.log('если вызвалась консоль нужно просмотреть алгоритм работы');
    // debugger;
    return;
  }
  ...
  for (let i = 0; i < object.children.length; i++) {
    ...
    if (nextPointLine) {
      ...
      if (!line || line.userData.weDoneWithThisLine) {
        pieceOfFreeSpace[0] = [];
        console.log('линия использовалась в предедущих полостях');
        // такого не должно быть, но иногда возможно, ошыбка в поиске пути.
        return;
      }
      ...
      if (line.userData.collisionPointsInf) {
        ...
        if (closesPoint) {
          if (
            ...
            console.log('точки которые уже использовались рание');
          }
  ...
  freeSpacesAll.forEach((lineGroup, i) => {
    ...
    lineGroup[0].forEach(line => {
      ...
      if (line.geometry.type === 'Geometry') {
        ...
        newLine.material.color.set(new THREE.Color(0x000000));
      } else if (line.geometry.type === 'CircleGeometry') {
        // // debugger;
  ...
  freeSpacesAll.forEach((lineGroup, i) => {
    if (lineGroup[1].length < 2) {
      ...
      res = sceneService.createObject(
        ...
        0.0001,
        'Free space',
        editor.voidSearchOptions.minArea,
        scene.getObjectByName('VoidsLayer'),
        i
      );
...
const isVoid = (line, linePoint, editor) => {
  let isVoid = true;
  let pointIndex = line.userData.nextPointIndex;
  let objects = editor.scene.getObjectByName('Objects');
  objects.children.forEach(object => {
    if (GeometryUtils.insidePolygon(
      object.userData.edgeModel.regions[0].path,
      linePoint) &&
      line.parent !== object
    ){
...
const weFindVoid = (lines) => {
  lines.forEach(line => {
    line.userData.weDoneWithThisLine = true;
  });
  console.log ('we find void');
  // отрабатывает при удачном нахождении полости
  return;
};

// let cutLine = (lineGroup) => {
//   // let newLine = [];
//   // // console.log(line);
//   // // // debugger;
...
//       let beforeIndex = i === 0? lineGroup.length - 1 : i-1;
//       let intersect = GeometryUtils.entitiesIntersectInfo(
//         lineGroup[beforeIndex],
//         lineGroup[i],
//         0.001);
//       // debugger;
//       if (intersect) {
//
//         let wayPoints = [
//           sceneService.findWayPoint(lineGroup[beforeIndex]),
//           sceneService.findWayPoint(lineGroup[i])
//         ];
//         console.log ('way to interPoint');
//         intersect.points.forEach(interPoint => {
//           let checker = 0;
//           wayPoints.forEach(wayPoint => {
//             wayPoint.forEach(point => {
//               console.log (GeometryUtils.getDistance(interPoint.point, point));
//               if (GeometryUtils.getDistance(interPoint.point, point) < 0.001) {
//                 checker += 1;
//               }
//             });
//           });
