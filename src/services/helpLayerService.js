...
let lengthNewLine = (editor, event) => {
  ...
  // if (!points) {
    ...
    if (lines[0][0].x === lines[1][0].x && lines[0][0].y === lines[1][0].y) {
      ...
      points[1] = lines[1][1];
      // debugger;
    } else if (lines[0][1].x === lines[1][0].x && lines[0][1].y === lines[1][0].y) {
      points[0] = lines[0][0];
      points[2] = lines[0][1];
      points[1] = lines[1][1];
      // debugger;
    } else if (lines[0][1].x === lines[1][1].x && lines[0][1].y === lines[1][1].y) {
      points[0] = lines[0][0];
      points[2] = lines[1][1];
      points[1] = lines[1][0];
      // debugger;
    } else if (lines[0][0].x === lines[1][1].x && lines[0][0].y === lines[1][1].y) {
      points[0] = lines[0][1];
      points[2] = lines[1][1];
      points[1] = lines[1][0];
      // debugger;
  ...
  let length0 = lengthLine (editor.activeEntities[0].geometry.vertices [0],
    editor.activeEntities[0].geometry.vertices [1]);
  let length1 = lengthLine (editor.activeEntities[1].geometry.vertices [0],
    ...
    if (lines[checkerI][0].x === points[2].x &&lines[checkerI][0].y === points[2].y){
      points[checkerI] = foundNewPoint (points[2], lines[checkerI][1], newLineLength);
    } else if (lines[checkerI][1].x === points[2].x && lines[checkerI][1].y === points[2].y){
      points[checkerI] = foundNewPoint (points[2], lines[checkerI][0], newLineLength);
    }
  ...
  if (!pointCurveCenter) {
    ...
    geometryLine.vertices.push(new THREE.Vector3(curveCenter.x, curveCenter.y, 0));
    ...
    pointCurveCenter = positionInLine (editor, helpCenterLine.geometry.vertices, clickResult.point);
    ...
    pointCurveCenter = positionInLine (editor, helpCenterLine.geometry.vertices, clickResult.point, pointCurveCenter);
    return pointCurveCenter;
  }
};
...
let positionInLine = (editor, lineWithPoint, pointB, point) =>{
  ...
  if (point) {
    ...
    let lineCD = ((lineAC * lineAC) - (lineAB * lineAB) + (lineBC * lineBC)) / (2 * lineAC);
    let pointD = foundNewPoint(pointC, pointA, lineCD);

    if (((pointA.x <= pointD.x && pointC.x >= pointD.x) || (pointA.x >= pointD.x && pointC.x <= pointD.x)) &&
      ((pointA.y <= pointD.y && pointC.y >= pointD.y) || (pointA.y >= pointD.y && pointC.y <= pointD.y))) {
...
let selectCenterPoint = (editor) =>{
  ...
  activeEntities.forEach((line, i) => {
    if (line.geometry.vertices[0].x === pointB.x && line.geometry.vertices[0].y === pointB.y){
      cutPoint[i] = foundNewPoint(line.geometry.vertices[0], line.geometry.vertices[1], cutLength);
      changeGeometry([line], [0], cutPoint[i], scene, editor);
    } else {
      cutPoint[i] = foundNewPoint(line.geometry.vertices[1], line.geometry.vertices[0], cutLength);
