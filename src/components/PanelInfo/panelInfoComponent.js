...
import { MEASUREMENT_ANGLE, MEASUREMENT_LINE, MEASUREMENT_POINT, MEASUREMENT_RADIAL } from '../../actions/measurement'
  ...
  render () {
    ...
    return (
      <div id='panel-info'>
        ...
        {selectMode === MEASUREMENT_RADIAL && <div>
          <h4>Measurement radial</h4>
          {(measurement.radial.line && measurement.radial.line.id) ?
            <Fragment>
              <p>Line:&nbsp;
                <b>id: </b>{measurement.radial.line.id}&nbsp;
                <b>Parent: </b>{measurement.radial.line.parent.name}
              </p>
              <p><b>Radius: </b>{measurement.radial.line.geometry.parameters.radius}</p>
            </Fragment>
            : <p>Chose line</p>}
        </div>}
        ...
        {selectMode === MEASUREMENT_ANGLE && <div>
          ...
          {(measurement.angle.angleValue) ?
            <Fragment>
              <p>
                <b>Angle 1: </b>{measurement.angle.angleValue[0].toFixed(5)} degree
              </p>
              <p>
                <b>Angle 2: </b>{measurement.angle.angleValue[1].toFixed(5)} degree
              </p>
            </Fragment>
            : null}
        </div>}
