  ...
  render () {
    ...
    return (
      <div className='form-horizontal form-custom container'>
        <span>Order created: {new Date(this.props.createdAt).toLocaleDateString(this.props.lang, dateOptions)}</span>
        <div>
          <legend>Objects information</legend>
          <ul className='polyamide-list'>
            {orderObjects && orderObjects.map((item, i) => {
              return (
                <li className='list-group-flush' key={i}>
                  <FormattedMessage id='calculatePrice.modal.object' defaultMessage='Object'>
                    {value => <h4>{value}: {item.object.name}</h4>}
                  </FormattedMessage>
                  < FormattedMessage
                    id='calculatePrice.modal.material'
                    defaultMessage='Material'>
                    {value => <h3>{value}: {item.object.userData.material.name}</h3>}
                  </FormattedMessage>
                  <FormattedMessage id='calculatePrice.modal.width' defaultMessage='Width'>
                    {value =>
                      <span>{value}: {Number(item.object.userData.info.width.toFixed(4))} mm</span>
                    }
                  </FormattedMessage>
                  <FormattedMessage id='calculatePrice.modal.height' defaultMessage='Height'>
                    {value =>
                      <span>{value}: {Number(item.object.userData.info.height.toFixed(4))} mm</span>
                    }
                  </FormattedMessage>
                  <FormattedMessage id='calculatePrice.modal.area' defaultMessage='Area'>
                    {value =>
                      <span>{value}: {Number(item.object.userData.info.area.toFixed(4))} mm2</span>
                    }
                  </FormattedMessage>
                  <FormattedMessage id='calculatePrice.modal.weight' defaultMessage='Weight'>
                    {value =>
                      <span>{value}: {Number(item.object.userData.info.weight).toFixed(4)} kg/m</span>
                    }
                  </FormattedMessage>
                  {item.object.userData.price &&
                  <FormattedMessage id='calculatePrice.modal.price' defaultMessage='Unit price'>
                    {value =>
                      <span>{value}: {item.object.userData.price}</span>
                    }
                  </FormattedMessage>}
                  <span className='profile col-sm-4 .col-md-4 .col-xs-4'>{consoleUtils.getSvg(item.object)}</span>

                  <FormattedMessage id='calculatePrice.modal.laserMarking' defaultMessage='Laser marking'>
                    {value =>
                      <span>{value}: {
                        item.object.userData.options.laserMarking ? 'true' : 'false'
                      }</span>
                    }
                  </FormattedMessage>
                  ...
                  <div>
                    <FormattedMessage id='calculatePrice.modal.type' defaultMessage='Type'>
                      {value =>
                        <span>{value}: {item.object.userData.options.type}</span>
                      }
                    </FormattedMessage>
                    <FormattedMessage id='calculatePrice.modal.color' defaultMessage='Color'>
                      {value =>
                        <span>{value}: {item.object.userData.options.color}</span>
                      }
                    </FormattedMessage>
                  </div>}

                  <FormattedMessage id='calculatePrice.modal.length' defaultMessage='Length'>
                    {value =>
                      <span>{value}: {item.object.userData.options.length}</span>
                    }
                  </FormattedMessage>

                  <FormattedMessage id='calculatePrice.modal.orderQty' defaultMessage='Order quantity'>
                    {value =>
                      <span>{value}: {item.object.userData.options.orderQty}</span>
                    }
                  </FormattedMessage>
                </li>)
            })}
