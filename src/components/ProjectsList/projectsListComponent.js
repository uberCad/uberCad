  ...
  <FormattedMessage id={id} defaultMessage={defaultMessage}>
    {title =>
      <Fragment>
        ...
        {title === sortFieldName ? <span className={sortUp ? 'fa fa-level-up' : 'fa fa-level-down'}/> : null }
      </Fragment>
    }
  </FormattedMessage>
)
      ...
      <div className='data-projects'>
        <Row className='table-head'>
          ...
          <Col xs={3} sm={2} className='hidden-sm-down table-head-name'>
            <TitleField id='projects.createdBy'
          ...
          projects.map((project, i) =>
            <Row key={i} className='table-row'>
              ...
              <Col xs={3} className='table-data title'>
                <Link to={`${process.env.PUBLIC_URL}/project/${project._key}`}>{project.title}</Link>
