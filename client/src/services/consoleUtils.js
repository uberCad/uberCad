...
let previewObjectInConsole = object => {
  let svg =  `<?xml version="1.0" encoding="UTF-8"?>
                            ...
                            <g id="group_d">
                                ${`<path d="${object.userData.edgeModel.svgData.pathD} " style="fill:rgb(200,240,200);stroke:black;stroke-width:0.00001">
...
let previewPathInConsole = (path, vertex) => {
  let minX = Math.min(...path.map(v => v.x)),
    minY = Math.min(...path.map(v => v.y)),
    width = Math.max(...path.map(v => v.x)) - minX,
    height = Math.max(...path.map(v => v.y)) - minY;
  ...
  if (vertex) {
    if (!Array.isArray(vertex)) {
      ...
      vertexData = `<circle cx="${(vertex.x / 1000).toFixed(4)}" cy="${(vertex.y / 1000).toFixed(4)}" r="0.0005" style="fill:rgb(150,255,150); stroke:black;stroke-width:0.00001" />`;
    } else {
